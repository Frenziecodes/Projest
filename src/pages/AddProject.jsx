    ...
    <section className='mb-16 mt-10'>
      <div className='flex md:pl-10 flex-col items-center ml-1 h-[100%] pb-8 mb-8 pt-10 Context'>
        <form onSubmit={handleSubmit(onSubmit)}>
          <h2 className="font-medium leading-tight lg:w-[600px] lg:text-2xl sm:text-md mt-10 md:mt-[50px] mb-4 m-auto text-gray-700">Add your project details by filling the form below</h2>
          <div className='flex flex-col md:flex-row justify-start items-start w-[100%]'>
            <div className='flex flex-col items-center md:items-end justify-center w-[100%] md:w-[40%] lg:w-[50%]'>
              <input type="text" placeholder='Project Title' {...register("title")} className='placeholder:text-slate-500 block bg-gray-200 w-[90vw] md:w-[36vw] lg:h-12 lg:w-[32vw] rounded-md my-4 py-2 pl-4 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1' onChange={(e) => setTitle(e.target.value)}></input>
              <p className='text-red-500'>{errors.title?.message}</p>
              <textarea placeholder='Project Description' {...register("description")} className='placeholder:text-slate-500 block bg-gray-200 min-h-[170px] h-48 w-[90vw] md:w-[36vw] lg:w-[32vw] rounded-md mb-6 py-2 pl-4 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1' onChange={(e) => setDescription(e.target.value)}></textarea>
            ...
            <div className='md:ml-5 w-11/12 h-64 md:w-2/3 lg:w-1/2 my-5 mx-5 md:my-0'>
              <input type="text" placeholder='User Github Link' {...register("userGithubLink")} className='placeholder:text-slate-500 block bg-gray-200 w-[90vw] md:w-[36vw] lg:w-[32vw] rounded-md my-4 py-2 pl-4 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 lg:h-12' onChange={(e) => setGithubLink(e.target.value)}></input>
              <p className='text-red-500'>{errors.userGithubLink?.message}</p>
              <input type="text" placeholder='Project Github Link' {...register("projectGithubLink")} className='placeholder:text-slate-500 block bg-gray-200 w-[90vw] md:w-[36vw] lg:w-[32vw] rounded-md my-4 py-2 pl-4 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 lg:h-12' onChange={(e) => setProjectLink(e.target.value)}></input>
              <p className='text-red-500'>{errors.projectGithubLink?.message}</p>
              <input type="text" placeholder='Demo Link' {...register("demoLink")} className='placeholder:text-slate-500 block bg-gray-200 w-[90vw] md:w-[36vw] lg:w-[32vw] rounded-md my-4 py-2 pl-4 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 lg:h-12' onChange={(e) => setDemoLink(e.target.value)}></input>
              <p className='text-red-500'>{errors.demoLink?.message}</p>
              <select
                {...register("category")}
                value={category}
                onChange={(e) => setCategory(e.target.value)}
                className='block bg-gray-200 w-[90vw] md:w-[36vw] lg:w-[32vw] rounded-md my-4 sm:my-4 py-2 pl-4 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 lg:h-12 text-slate-500'
              >
          ...
          <div className='w-52 mt-16 lg:my-16 m-auto flex'>
            <button type='submit' className='bg-blue-800 hover:bg-blue-700 w-28 h-10 text-white font-bold py-2 px-4 rounded'>
              Submit
            </button>
            <button onClick={() => navigate('/viewprojects')} className='bg-red-500 hover:bg-red-700 w-28 h-10 text-white font-bold py-2 px-4 rounded ml-4'>
              Cancel
            </button>
          </div>
          {uploadProgress > 0 && (
