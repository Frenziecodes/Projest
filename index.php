<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Projest</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="assets/css/style.css">
    <link href="/dist/output.css" rel="stylesheet">
</head>
<body>
    <!-- navbar -->
    <div class="flex justify-between items-center align-middle h-16 w-full bg-violet-500 text-white py-2 px-3 fixed top-0 left-0 right-0 z-30 mb-16">
        <div class="flex justify-center items-center">
           PROJEST
        </div>
        <ul class="flex justify-center items-center">
            <li class="m-2 md:m-5 bg-indigo-900 px-4 py-1 rounded-sm"><a href="#">Home</a></li>
            <li class="dropdown m-2 md:m-5">
                <a href="#">Projects</a>
                <!-- <ul class="dropdown-content p-3 bg-violet-500 absolute">
                  <li class="m-2"><a href="#">View Project</a></li>
                  <li class="m-2"><a href="#">Add Project</a></li>                  
                </ul> -->
              </li>         
            <li class="m-2 md:m-5"><a href="#">Contributing</a></li>
            <li class="inline-block px-3 py-2 m-2 border-2 border-white text-white font-medium text-sm leading-snug uppercase rounded-full hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out md:m-5" data-mdb-ripple="true" data-mdb-ripple-color="light"><a href="login.php">Sign in</a></li>
        </ul>        
        
    </div>
      
  <div class="container my-20 px-6 mx-auto">    
        <section class="mb-16 text-gray-800 text-center lg:text-left">

        <div class="grid lg:grid-cols-2 gap-6 xl:gap-12 items-center">
            <div class="mb-2 lg:mb-0">
            <h2 class="text-3xl md:text-4xl xl:text-4xl font-bold tracking-tight">
                Ready to share your projects<br />
                <span class="text-violet-600">on projest?</span>
            </h2>
            </div>
            <div class="mb-4 lg:mb-0">            
            <p class="text-gray-500 text-lg">
                Join a community of like-minded individuals and showcase your skills, get recognized for your work and connect with potential collaborators, elevate your career with a professional portfolio on Projest, discover new projects and talents, and find your next job or team member on Projest.
            </p>
            </div>
        </div>

        </section>    
   </div>  

   <!-- Projest Features -->
    <div class="container my-10 px-6 mx-auto">    
            <section class="mb-20 text-gray-800">
                <div class="flex justify-center">
                    <div class="text-center max-w-[700px]">
                    <h2 class="text-3xl font-bold mb-6">What makes Projest so great?</h2>
                    <p class="text-gray-500 mb-12">
                        Projest is a powerful platform that empowers users to showcase their work, connect with others in their field, and advance their careers.
                        To make it easy for users to navigate and find what they are looking for, Projest offers a wide range of features such as :
                    </p>
                    </div>
                </div>
            
                <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-x-6 xl:gap-x-12">
                    <div class="mb-6">
                    <div class="flex">
                        <div class="shrink-0 mt-1">
                        <svg class="w-4 h-4 text-violet-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path fill="currentColor"
                            d="M504 256c0 136.967-111.033 248-248 248S8 392.967 8 256 119.033 8 256 8s248 111.033 248 248zM227.314 387.314l184-184c6.248-6.248 6.248-16.379 0-22.627l-22.627-22.627c-6.248-6.249-16.379-6.249-22.628 0L216 308.118l-70.059-70.059c-6.248-6.248-16.379-6.248-22.628 0l-22.627 22.627c-6.248 6.248-6.248 16.379 0 22.627l104 104c6.249 6.249 16.379 6.249 22.628.001z">
                            </path>
                        </svg>
                        </div>
                        <div class="grow ml-4">
                        <p class="font-bold mb-1">Profile creation</p>
                        <p class="text-gray-500">You can create your personal profile to showcase your Github projects,skills and contact information.</p>
                        </div>
                    </div>
                    </div>
            
                    <div class="mb-6">
                    <div class="flex">
                        <div class="shrink-0 mt-1">
                        <svg class="w-4 h-4 text-violet-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path fill="currentColor"
                            d="M504 256c0 136.967-111.033 248-248 248S8 392.967 8 256 119.033 8 256 8s248 111.033 248 248zM227.314 387.314l184-184c6.248-6.248 6.248-16.379 0-22.627l-22.627-22.627c-6.248-6.249-16.379-6.249-22.628 0L216 308.118l-70.059-70.059c-6.248-6.248-16.379-6.248-22.628 0l-22.627 22.627c-6.248 6.248-6.248 16.379 0 22.627l104 104c6.249 6.249 16.379 6.249 22.628.001z">
                            </path>
                        </svg>
                        </div>
                        <div class="grow ml-4">
                        <p class="font-bold mb-1">Personalized Dashboard</p>
                        <p class="text-gray-500">Have your own personalized Dashboard where you can see your own projects,projects you liked or shared e.tc</p>
                        </div>
                    </div>
                    </div>
            
                    <div class="mb-6">
                    <div class="flex">
                        <div class="shrink-0 mt-1">
                        <svg class="w-4 h-4 text-violet-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path fill="currentColor"
                            d="M504 256c0 136.967-111.033 248-248 248S8 392.967 8 256 119.033 8 256 8s248 111.033 248 248zM227.314 387.314l184-184c6.248-6.248 6.248-16.379 0-22.627l-22.627-22.627c-6.248-6.249-16.379-6.249-22.628 0L216 308.118l-70.059-70.059c-6.248-6.248-16.379-6.248-22.628 0l-22.627 22.627c-6.248 6.248-6.248 16.379 0 22.627l104 104c6.249 6.249 16.379 6.249 22.628.001z">
                            </path>
                        </svg>
                        </div>
                        <div class="grow ml-4">
                        <p class="font-bold mb-1">Collaboration Tools</p>
                        <p class="text-gray-500">You can collaborate with others by commenting on their projects.</p>
                        </div>
                    </div>
                    </div>
            
                    <div class="mb-6">
                    <div class="flex">
                        <div class="shrink-0 mt-1">
                        <svg class="w-4 h-4 text-violet-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path fill="currentColor"
                            d="M504 256c0 136.967-111.033 248-248 248S8 392.967 8 256 119.033 8 256 8s248 111.033 248 248zM227.314 387.314l184-184c6.248-6.248 6.248-16.379 0-22.627l-22.627-22.627c-6.248-6.249-16.379-6.249-22.628 0L216 308.118l-70.059-70.059c-6.248-6.248-16.379-6.248-22.628 0l-22.627 22.627c-6.248 6.248-6.248 16.379 0 22.627l104 104c6.249 6.249 16.379 6.249 22.628.001z">
                            </path>
                        </svg>
                        </div>
                        <div class="grow ml-4">
                        <p class="font-bold mb-1">Search Functionality</p>
                        <p class="text-gray-500">You can search for specific projects or keywords within the projects description.</p>
                        </div>
                    </div>
                    </div>
            
                    <div class="mb-6">
                    <div class="flex">
                        <div class="shrink-0 mt-1">
                        <svg class="w-4 h-4 text-violet-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path fill="currentColor"
                            d="M504 256c0 136.967-111.033 248-248 248S8 392.967 8 256 119.033 8 256 8s248 111.033 248 248zM227.314 387.314l184-184c6.248-6.248 6.248-16.379 0-22.627l-22.627-22.627c-6.248-6.249-16.379-6.249-22.628 0L216 308.118l-70.059-70.059c-6.248-6.248-16.379-6.248-22.628 0l-22.627 22.627c-6.248 6.248-6.248 16.379 0 22.627l104 104c6.249 6.249 16.379 6.249 22.628.001z">
                            </path>
                        </svg>
                        </div>
                        <div class="grow ml-4">
                        <p class="font-bold mb-1">Social media sharing</p>
                        <p class="text-gray-500">Share your projects on social media platforms such as Twitter,Linkedin and Facebook</p>
                        </div>
                    </div>
                    </div>
            
                    <div class="mb-6">
                    <div class="flex">
                        <div class="shrink-0 mt-1">
                        <svg class="w-4 h-4 text-violet-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path fill="currentColor"
                            d="M504 256c0 136.967-111.033 248-248 248S8 392.967 8 256 119.033 8 256 8s248 111.033 248 248zM227.314 387.314l184-184c6.248-6.248 6.248-16.379 0-22.627l-22.627-22.627c-6.248-6.249-16.379-6.249-22.628 0L216 308.118l-70.059-70.059c-6.248-6.248-16.379-6.248-22.628 0l-22.627 22.627c-6.248 6.248-6.248 16.379 0 22.627l104 104c6.249 6.249 16.379 6.249 22.628.001z">
                            </path>
                        </svg>
                        </div>
                        <div class="grow ml-4">
                        <p class="font-bold mb-1">Like and Share</p>
                        <p class="text-gray-500">You can like and share projects that you find interesting.</p>
                        </div>
                    </div>
                    </div>
            
                    <div class="mb-6">
                    <div class="flex">
                        <div class="shrink-0 mt-1">
                        <svg class="w-4 h-4 text-violet-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path fill="currentColor"
                            d="M504 256c0 136.967-111.033 248-248 248S8 392.967 8 256 119.033 8 256 8s248 111.033 248 248zM227.314 387.314l184-184c6.248-6.248 6.248-16.379 0-22.627l-22.627-22.627c-6.248-6.249-16.379-6.249-22.628 0L216 308.118l-70.059-70.059c-6.248-6.248-16.379-6.248-22.628 0l-22.627 22.627c-6.248 6.248-6.248 16.379 0 22.627l104 104c6.249 6.249 16.379 6.249 22.628.001z">
                            </path>
                        </svg>
                        </div>
                        <div class="grow ml-4">
                        <p class="font-bold mb-1">Reviews and Ratings</p>
                        <p class="text-gray-500">Rate and review projects and give feedback a feedback to the project Creator.</p>
                        </div>
                    </div>
                    </div>
            
                    <div class="mb-6">
                    <div class="flex">
                        <div class="shrink-0 mt-1">
                        <svg class="w-4 h-4 text-violet-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path fill="currentColor"
                            d="M504 256c0 136.967-111.033 248-248 248S8 392.967 8 256 119.033 8 256 8s248 111.033 248 248zM227.314 387.314l184-184c6.248-6.248 6.248-16.379 0-22.627l-22.627-22.627c-6.248-6.249-16.379-6.249-22.628 0L216 308.118l-70.059-70.059c-6.248-6.248-16.379-6.248-22.628 0l-22.627 22.627c-6.248 6.248-6.248 16.379 0 22.627l104 104c6.249 6.249 16.379 6.249 22.628.001z">
                            </path>
                        </svg>
                        </div>
                        <div class="grow ml-4">
                        <p class="font-bold mb-1">Contact feature</p>
                        <p class="text-gray-500">You can contact project creators via a contact form or contact details.</p>
                        </div>
                    </div>
                    </div>
                </div>
            </section>
    </div>

<div class="container my-12 px-6 mx-auto">

    <section class="mb-4">
      <style>
        @media (min-width: 992px) {
          #cta-img-nml-50 {
            margin-left: 50px;
          }
        }
      </style>
  
      <div class="flex flex-wrap">
        <div class="grow-0 shrink-0 basis-auto w-full lg:w-5/12 mb-12 lg:mb-0">
          <div class="flex lg:py-12">
            <img src="assets/img/home.jpg" class="w-full rounded-lg shadow-lg"
              id="cta-img-nml-50" style="z-index: 10" alt="" />
          </div>
        </div>
  
        <div class="grow-0 shrink-0 basis-auto w-full lg:w-7/12">
          <div
            class="bg-violet-600 h-full rounded-lg p-6 lg:pl-12 text-white flex items-center text-center lg:text-left">
            <div class="lg:pl-12">
              <h2 class="text-3xl font-bold mb-6">What are you waiting for?</h2>
              <p class="mb-6 pb-2 lg:pb-0">
                Sign up now and start showcasing your projects, connecting with like-minded individuals, and advancing your career on Projest
              </p>
              <button type="button" class="inline-block px-7 py-3 border-2 border-white text-white font-medium text-sm leading-snug uppercase rounded-full hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out" data-mdb-ripple="true" data-mdb-ripple-color="light">
                    <a href="login.php">Sign up now</a>
              </button>
            </div>
          </div>
        </div>
      </div>
    </section>   
  
  </div> 

  <footer class="bg-violet-600 text-center lg:text-left relative bottom-0 right-0 left-0">
    <div class="text-white text-center p-4">
      © 2023 Copyright:
      <a href="#">PROJEST</a>
    </div>
  </footer>
  
    <script src="assets/js/app.js"></script>
</body>
</html>